<template>
	<view class="rf-search-bar" :class="'bg-' + themeColor.name">
		<view class="header" :style="{width:merchantShow ? '100vw' : width+'px', marginTop: merchantShow ? 0 : inputTop+'px'}">
			<!-- 扫一扫 -->
			<!-- <view class="addr">
				<text class="iconfont" @tap.stop="link" :class="[icon]"></text>
			</view> -->
			<!-- 搜索框 -->
			<view class="input-box" @tap="toSearch" style="margin-right:0;">
				<input disabled />
				<view class="wrapper">
	...
	.rf-search-bar {
		...
		.header {
			...
			.input-box {
				...
				display: flex;
				align-items: center;
				margin: 6upx $spacing-base 0 $spacing-base;
