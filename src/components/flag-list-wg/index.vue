				...
				<view class="cu-card dynamic" :class="isCard ? 'no-card' : ''">
					<view class="cu-item shadow">
						...
						>
							...
							<!-- <view @click="wxShare()" class="flex-1 flex middle">
                <view class="cuIcon-messagefill"></view>
                <view class="margin-lr-xs"> 邀请围观</view>
              </view> -->
							<flag-send
								:id="item.flag_id"
...
<script>
import { clickGive, getMakeMoney, getCommentPl, flaglist } from '@/api/basic';
export default {
	props: {
		// list: {
		// 	type: Array,
	...
	methods: {
		...
		actionSheetCallback(index) {
			...
			//   console.log(index);
			if (index == 0) {
				this.$mflagShare.shareFriend();
			} else if (index == 1) {
				this.$mflagShare.shareFriendcricle();
			}
		},
		clickCur(idx) {
			this.wgidx = idx;
