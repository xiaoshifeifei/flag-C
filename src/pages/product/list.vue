	...
	<view class="container product-list">
		<view class="rf-header-box">
			<view class="rf-header" :style="{width:width+'px',height:height+'px'}">
				...
				<view class="pageTitle">{{typeName}}</view>

			</view>
		</view>
		<view class="rf-header-screen" :style="{top:height+'px'}">
			<view class="rf-screen-top" style="display: none;">
				...
				<view class="rf-dropdownlist" :class="[selectH>0?'rf-dropdownlist-show':'']" :style="{height:selectH+'upx'}">
					<view class="rf-dropdownlist-item rf-icon-middle" :class="[item.selected?'rf-bold':'']" v-for="(item,index) in dropdownList"
		...
		<view class="product-list-wrapper">
			...
			<rf-product-list :bottom="0" :list="productList" :isList="isList" :style="{paddingTop: 100 + 'upx' }"></rf-product-list>
		</view>
			...
			<scroll-view class="rf-scroll-box" scroll-y :scroll-top="scrollTop">
				...
				<view class="rf-drop-item rf-icon-middle" :class="[item.selected?'rf-bold':'']" v-for="(item,index) in attrData"
				 :key="index" @tap.stop="btnSelected" :data-index="index">
					<text class="iconfont icongouxuan" :class="'text-' + themeColor.name" :size="16" :bold="true" v-if="item.selected"
					 rf-icon-class="rf-middle"></text>
					<text class="rf-ml rf-middle">{{item.name}}</text>
				</view>
			...
			<view class="rf-drawer-box" :style="{paddingTop:height+'px'}">
				<scroll-view class="rf-drawer-scroll" scroll-y :style="{height:drawerH+'px'}">
					...
					<view class="rf-drawer-content rf-flex-attr">
						<view class="rf-attr-item" :class="[item.isActive ? `bg-${themeColor.name} rf-btmItem-active` : 'rf-btmItem-normal']"
						 v-for="(item, index) in productCateList" :key="item.id" @tap.stop="cateBtnSelected(index)">
					...
					<view class="rf-drawer-content rf-flex-attr">
						<view class="rf-attr-item" :class="[item.isActive ? `bg-${themeColor.name} rf-btmItem-active` : 'rf-btmItem-normal']"
						 v-for="(item, index) in brandList" :key="item.id" @tap.stop="brandBtnSelected(index)">
							<view class="rf-attr-ellipsis">{{ item.title }}</view>
						</view>
					</view>
				...
				<view class="rf-attr-btnbox">
					<view class="rf-attr-safearea">
						<view class="rf-drawer-btn rf-drawerbtn-black" :class="'text-' + themeColor.name" hover-class="rf-white-hover"
	...
	export default {
		...
		methods: {
			...
			initData(options) {
				...
				this.typeName = '搜索';
				if (options.cate_id) {
					params.categoryId = options.cate_id;
					this.categoryId = options.cate_id;
				}
				if (options.isSpecial) {
					params.isSpecial = options.isSpecial;
				}
				if (options.param) {
					params = { ...params,
			...
			dealwidthTypename() {
				let categorys = {
					id: '',
					label: '',
					children: JSON.parse(JSON.stringify(this.$mStore.getters.categories))
				};
				this.typeName = traverseTree(categorys, parseInt(this.categoryId), 'id', 'label');
			...
			async getProductList(type) {
				await this.$http
					.get(`${productSearchList}`, {
						//...this.productParams,
						productName:this.productParams.keyword,
						pageNum: this.page,
