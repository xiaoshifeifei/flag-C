<template>
	<view class="rf-category">
		...
		<view class="input-box">
			...
			<view class="pageTitle">分页专区</view>
		</view>
		<view class="category-list" @touchmove.stop.prevent="moveHandle" v-if="(styleCateType === 'one_two_three_cover' || styleCateType === 'one_two_three_text') && categoryList.length > 0">
			<!-- 左侧分类导航 -->
			<scroll-view scroll-y="true" class="left">
				<view class="row" v-for="(fItem, fIndex) in categoryList" :key="fItem.id" :class="[fIndex === showCategoryIndex ? 'on' : '']"
			...
			<scroll-view scroll-y="true" class="right">
				<view v-for="(fItem, fIndex) in categoryList" :key="fItem.id">
					<view v-if="fIndex === showCategoryIndex" class="category">
						...
						<view class="box" v-for="sItem in fItem.children" :key="sItem.id">
							<view class="second-category-text" @tap="navTo(`/pages/product/list?cate_id=${sItem.id}&isSpecial=${sItem.isSpecial}`)">
								{{ sItem.label }}
								<text class="iconfont iconyou"></text>
							</view>
							<view class="list" v-if="sItem.children && sItem.children.length > 0">
								<view class="box" v-for="tItem in sItem.children" :key="tItem.id" @tap.stop="navTo(`/pages/product/list?cate_id=${tItem.id}&isSpecial=${sItem.isSpecial}`)">
									<block v-if="styleCateType === 'one_two_three_cover'">
										<image :src="tItem.cover || errorImage"></image>
										<view class="text">{{ tItem.label }}</view>
									</block>
						...
						<view class="no-data" v-if="fItem.children.length === 0">
							<i class="iconfont icon404"></i>
							暂无分类
						</view>
					</view>
...
export default {
  ...
  watch: {
    '$mStore.getters.categories': function () {
      this.categoryList = this.$mStore.getters.categories
    }
  },
  // 每次页面显示 执行该方法
  onLoad () {
    ...
    async getProductCate (type) {
      await this.$http
        ...
        .then(async r => {
          ...
          const cateId = uni.getStorageSync('indexToCateId')
          if (cateId) {
