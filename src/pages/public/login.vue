<template>
	<view class="login">
		...
		<view class="login-type-2" v-if="styleLoginType === 'two'">
			...
			<view class="login-type-content">
				<!-- <image
					...
					:src="loginBg"
					:style="{
						height: tabCurrentIndex === 1 ? '150vw' : '94vw'
					}"
				></image> -->
				<view class="main bg-white shadow">
					<view class="nav-bar">
						<view
							class="nav-bar-item"
							...
							:class="
								tabCurrentIndex === index
									? `text-${themeColor.name} nav-bar-item-active`
									: ''
							"
							@tap="tabClick(index)"
						>
					...
					<block v-if="tabCurrentIndex === 0">
						<view class="login-type-form">
							<view class="input-item">
								<text
									class="iconfont iconzhanghuffffffpx"
							...
							<view class="input-item" v-if="loginByPass">
								<text
									class="iconfont iconmimaffffffpx"
									:class="'text-' + themeColor.name"
								></text>
								<input
									class="login-type-input"
					...
					<block v-if="tabCurrentIndex === 1">
						<view class="login-type-form">
							...
							<view class="input-item input-item-sms-code">
								...
								<view class="input-wrapper">
									...
									<button
										class="sms-code-btn"
										:disabled="smsCodeBtnDisabled"
										@tap.stop="getSmsCode('register', registerParams.username)"
									>
										<text v-if="!smsCodeBtnDisabled">获取验证码</text>
										<text v-else class="sms-code-resend">{{
											`重新发送 (${codeSeconds})`
							...
							<view
								class="input-item"
								v-if="!closeRegisterPromoCode"
							>
								<text
									class="iconfont iconyanzhengma1"
									:class="'text-' + themeColor.name"
								></text>
								<input
									class="login-type-input"
		...
		<rf-box
			...
			@ok="downApp"
		>
			<view>恭喜您！注册成功。赶紧下载吧！</view>
			<view>下载地址：{{ downPath }}</view>
		</rf-box>
	</view>
</template>
<script>
...
import { profileme } from '@/api/userInfo';
...
export default {
	...
	computed: {
		downPath() {
			return this.$mConstDataConfig.downPath;
		}
	},
	...
	onLoad(options) {
		...
		if (options && options.invitecode) {
			this.registerParams.promoCode = options.invitecode;
		}
		this.loginParams.username = uni.getStorageSync('loginusername') || '';
	...
	methods: {
		...
		async getSmsCode(usage = 'login') {
			...
			await this.$http
				.post(usage !== 'register' ? smsLogCode : smsRegCode, {
					username: this.reqBody.username
				})
				.then(r => {
					console.log(r);
					if (r.code != 0) {
						this.$mHelper.toast('验证码发送失败，' + r.msg);
						return;
					}
		...
		async toLogin() {
			...
			if (this.loginByPass) {
				...
				this.reqBody.verify = this.loginParams.code;
				loginApi = loginByPass;
				cheRes = this.$mGraceChecker.check(
					this.reqBody,
		...
		async handleLogin(params, loginApi) {
			...
			await this.$http
				...
				.then(r => {
					...
					if (r.status) {
						vm.$mHelper.toast(r.msg || '用户名或密码错误', 3000, true);
						return;
					}
					vm.$mHelper.toast('恭喜您，登录成功！', 3000, true);
				...
				.catch(e => {
					this.$mHelper.toast(e.msg || '用户名或密码错误', 3000, true);
					this.btnLoading = false;
				});
		},
		//下载
		downApp() {
			window.location.href = this.downPath;
		},
		...
		async toRegister() {
			...
			if (this.$mSettingConfig.closeRegister) {
				this.$mHelper.toast('暂未开放注册，敬请期待～', 3000, true);
				return;
			}
			this.reqBody.username = this.registerParams.username;
			this.reqBody.verify = this.registerParams.verify;
			...
			// ) {
			// 	this.$mHelper.toast('两次输入的密码不一致', 3000, true)
			// 	return
			// }
			...
			await this.$http
				...
				.then(r => {
					...
					this.btnLoading = false;
					if (r.code != 0) {
						vm.$mHelper.toast(r.msg || '注册失败，请重试', 3000, true);
						return;
					}

					r.data.username = this.reqBody.username;
					this.$mStore.commit('login', r.data);
					vm.$mHelper.toast('登录成功', 3000, true);
				...
				.catch(e => {
					vm.$mHelper.toast(e.msg || '注册失败，请重试', 3000, true);
					this.btnLoading = false;
				});
		}
	}
