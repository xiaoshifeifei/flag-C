				...
				<view class="wrapper">
					...
					<view class="input-content">
						<view class="input-item">
							...
							<input type="number" v-model="resetPasswordParams.username" placeholder="请输入手机号码" maxlength="11" />
						</view>
						<view class="input-item input-item-sms-code">
							<view class="input-wrapper">
								<view class="rf-input-wrapper">
									<view class="tit">验证码</view>
									<input type="number" v-model="resetPasswordParams.code" placeholder="请输入验证码" maxlength="4" data-key="mobile" />
								</view>
						...
						<view class="input-item">
							...
							<input v-model="resetPasswordParams.password" type="password" placeholder="请输入密码" maxlength="18" />
						</view>
						<view class="input-item">
							<text class="tit">确认密码</text>
							<input type="password" v-model="resetPasswordParams.password_repetition" placeholder="请输入确认密码" />
						</view>
					</view>
					<button class="confirm-btn" :class="'bg-' + themeColor.name" :disabled="btnLoading" :loading="btnLoading" @tap="toUpdatePassword">
						{{ type == 1 ? '修改密码' : '找回密码' }}
					</button>
				</view>
				...
				<view class="main">
					<view class="login-type-form">
						<view class="input-item">
							...
							<input class="login-type-input" type="number" name="mobile" v-model="resetPasswordParams.username" placeholder="请输入手机号码"
							 maxlength="11" />
						</view>
						<view class="input-item input-item-sms-code">
							...
							<view class="input-wrapper">
								<view class="rf-input-wrapper">
									<input type="number" class="login-type-input" v-model="resetPasswordParams.verification" placeholder="请输入验证码"
									 maxlength="6" />
								</view>
								<button class="sms-code-btn" :disabled="smsCodeBtnDisabled" @tap.stop="getSmsCode()">
									<text v-if="!smsCodeBtnDisabled">获取验证码</text>
						...
						<view class="input-item">
							...
							<input class="login-type-input" type="password" v-model="resetPasswordParams.password" :placeholder="'请输入'+pText+'密码'"
							 maxlength="20" />
						</view>
						<view class="input-item">
							<text class="iconfont iconmimaffffffpx" :class="'text-' + themeColor.name"></text>
							<input class="login-type-input" type="password" v-model="resetPasswordParams.password_repetition" :placeholder="'请确认'+pText+'密码'"
							 maxlength="20" />
						</view>
					</view>
					<view class="login-type-tips" v-if="type!=3 && type!=4">
...
import {payPassword} from "@/api/userInfo"
...
export default {
  ...
  computed:{
	  pageTitle(){
		  ...
		  let title = this.titleText[this.type]
		  uni.setNavigationBarTitle({title})
		  return title
	  },
	  pText(){
		  return this.type == 3 || this.type == 4 ? '支付' :'';
	  }
  },
  onLoad (options) {
    const time =
  ...
  methods: {
    // 获取手机验证码
    async getSmsCode () {
      this.reqBody.username = this.resetPasswordParams.username
      console.log(this.reqBody.username)
      const checkSendCode = this.$mGraceChecker.check(
        this.reqBody,
      ...
      await this.$http
        .post(smsChangePwdCode, {
          phone: this.reqBody.username
        })
        .then(r => {
    ...
    async toUpdatePassword () {
      ...
      this.reqBody.verification = this.resetPasswordParams.verification
      ...
      if (
        this.resetPasswordParams.password !== this.resetPasswordParams.password_repetition
      ) {
	  ...
	  if(this.type == 3 || this.type == 4){
		  let ds =  await this.$http.post(payPassword,this.reqBody);
		  this.btnLoading = false
		  if(!ds.status){
			  this.$mHelper.toast(this.type == 3  ? '修改成功' : '设置成功');
