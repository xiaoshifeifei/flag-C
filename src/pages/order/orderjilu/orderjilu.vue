<template>
	<view>
		<view v-if="list.length > 0" class="wrap">
			<view class="page-box">
				<view class="order" v-for="(res, index) in list" :key="index">
					<view class="top">
						<view class="left">
              ...
              <view v-if="res.flag_status==2" class="store">挑战购</view> -->
							<view class="store">{{ res.orderType }}</view>
					...
					<view class="item">
						...
						<view class="content">
							...
							<view class="delivery-time">下单时间 {{ res.paid_time }}</view>
						</view>
						<view class="right">
							<view class="price">
								￥{{ res.money / 100 }}
