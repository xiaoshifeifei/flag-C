<template>
	<view class="about" v-if="!loading">
		...
		<view class="shop-info " v-if="title === '注册协议'">
			<view class="about-content" v-if="detail.text_register">
				<rf-parser lazy-load :html="detail.text_register"></rf-parser>
				<button class="confirm-btn" :class="'bg-' + themeColor.name"  @tap="$mRouter.back()">
					返回
				</button>
			</view>
			<rf-empty
				:info="`暂无${title}`"
				v-if="!detail.text_register && !loading"
			></rf-empty>
		</view>
		<view class="shop-info" v-if="title === '隐私协议'">
			<view class="about-content" v-if="detail.text_privacy">
				<rf-parser lazy-load :html="detail.text_privacy"></rf-parser>
			</view>
			<rf-empty
				:info="`暂无${title}`"
	...
	methods: {
		...
		async getConfigList(field) {
			if (field.indexOf('text') !== -1) {
				this.detail = this.$mConstDataConfig.text;
				console.log(this.detail)
				this.loading = false
			}
			else if (field.indexOf('protocol') !== -1) {
				await this.$http
					.get(`${configList}`, {
						field
					})
