...
import { getAppUpdate } from '../../utils/appUpdate';
export default {
	data() {
		return {
			...
			notifyChecked: false,
			isNewVersion: uni.getStorageSync('isNewVersion')
				? uni.getStorageSync('isNewVersion')
				: false,
			colorModal: false,
	...
	onLoad() {
		var setList = uni.getStorageSync('bendiapp');
		// console.log('本地',setList)
			// console.log(setList.version.split())
		if (setList) {
			this.$mConstDataConfig.setList.forEach(item => {
				if (item.type == 1) {
					item.content = 'v' + setList.version;
				}
			});
			this.setList = this.$mConstDataConfig.setList;
		} else {
			this.setList = this.$mConstDataConfig.setList;
		}
	...
	methods: {
		...
		toLogout() {
			uni.showModal({
				...
				success: e => {
					if (e.confirm) {
						this.$http.post(`${logout}`, { id: 8 }).then(() => {
