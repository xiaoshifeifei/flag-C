<template>
	<view class="address-list">
		<view class="rf-list" v-if="addressList.length > 0">
			...
			>
				<view class="mid" @longtap="deleteAddress(item.addr_id)">
 ...
 * @copyright 2019
 */
import { addrlist } from '@/api/product.js';
import { addressDelete } from '@/api/userInfo.js';
...
export default {
	...
	methods: {
		...
		async handleAddressDelete(id) {
			await this.$http.post(`${addressDelete}`, { addr_id: id }).then(r => {
		...
		//       this.addressList = [...this.addressList, ...r.data.list]
		//     })
		//     .catch(() => {
		//       this.loading = false
		//       if (type === 'refresh') {
		...
		// 跳转添加地址页面
		addAddress(type, id, one, two, three) {
			console.log(one);
			console.log(two);
			console.log(three);

			this.$mRouter.push({
				route: `/pages/user/address/manage?type=${type}&id=${id}&qu=${one} ${two}${three}`
			});
		}
	}
};
