<template>
	<view class="pay-success" v-if="!loading">
		...
		<view class="pay-card">
			<view class="pay-card-money">{{moneySymbol}}{{ orderDetail.payment }}</view>
			<view class="item"><text class="tit">订单编号：</text>{{ orderDetail.orderNo }}</view>
			<view class="item"><text class="tit">下单时间：</text>{{ orderDetail.createTime | time }}</view>
		</view>
...
export default {
  ...
  methods: {
    ...
    navTo () {
      let route
      route = '/pages/order/order?index=-1'
