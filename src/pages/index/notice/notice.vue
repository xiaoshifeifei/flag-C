<template>
	<view clas="notice">
		<view class="notice-item" v-for="item in announceList" :key="item.noticeId">
			<text class="time">{{ item.updateTime | time }}</text>
			<view class="content" @tap="navTo(`/pages/index/notice/detail?id=${item.noticeId}`)">
				<text class="title">{{ item.noticeTitle }}</text>
				<view class="img-wrapper" v-if="item.image">
					<rf-image class="pic" :preview="false" :mode="'aspectFit'" :src="item.image"></rf-image>
				</view>
				<text class="introduce" v-if="item.remark">
					作者：{{ item.remark }}
				</text>
				<view class="bot b-t">
					<text>查看详情</text>
    ...
    async getNotifyAnnounceIndex (type) {
      await this.$http
        ...
        .then(r => {
          ...
          this.announceList = [...this.announceList, ...list]
        })
        .catch(() => {
          if (type === 'refresh') {
            uni.stopPullDownRefresh()
