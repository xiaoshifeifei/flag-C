<template>
	<view class="rf-notice-detail">
		...
		<view class="banner-title" v-if="!announceDetail.image">{{
			announceDetail.noticeTitle
		}}</view>
		<view class="article-meta" v-if="announceDetail.updateTime">
			<text class="article-author">{{ announceDetail.remark }}</text>
			<text class="article-text">发布于</text>
			<text class="article-time">{{ announceDetail.updateTime | time }}</text>
		</view>
...
export default {
  ...
  methods: {
    ...
    async getNotifyAnnounceView (id) {
      await this.$http
        .get(`${notifyAnnounceView}`, { noticeId: id })
        .then(r => {
          this.loading = false
